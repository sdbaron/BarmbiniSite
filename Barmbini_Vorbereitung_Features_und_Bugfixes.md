...
- **Journal (2026-09-08):** `/var/log/journal` von ~880 MB auf ~65 MB reduziert (`journalctl --vacuum-size=100M` + Drop-in `SystemMaxUse=100M` / `MaxRetentionSec=14day`). Disk `/` danach **68 %** (2.9 GB frei).
...
- Rollen-Pflege `class-roles.php` (Barmbini_Core_Roles): migriert die frühere Projektrolle „Verkäufer“ `barmbini_verkaeufer` idempotent zur WooCommerce-Standardrolle `shop_manager` (Nutzer umhängen + Alt-Rolle entfernen, `admin_init`-Selbstheilung); Standardrolle `editor`/„Redakteur“ bleibt unangetastet (Detail: `Tasks/Barmbini_Aufgabe_Rolle_Verkäufer.md`)
- Interne Anleitung `class-staff-guides.php` (Barmbini_Core_Staff_Guides): Frontend-Seite `/anleitung-shop-manager/` (ausführliche Schritt-für-Schritt-Anleitung), idempotent vom Plugin angelegt; berechtigt mit `barmbini_view_guide_shop_manager` (Administrator + Shop Manager); Login-/Fallback-Umleitung + `noindex`; Admin-Menüpunkt „Anleitungen“ + Admin-Bar-Link als Einstieg. **Seit 0.9.6 ist die Redakteur-Anleitung ausgeblendet** (Rolle `editor` wird nicht genutzt): Redakteur-Cap wird nicht mehr vergeben, Seite nicht geführt, Menü/Admin-Bar/Landing zeigen nur die Shop-Manager-Anleitung; reaktivierbar per Konstante `BARMBINI_GUIDE_REDAKTEUR_ENABLED` / Filter `barmbini_guide_redakteur_enabled`. Der alte Slug `/anleitung-verkaeufer/` wird seit 0.9.3 nicht mehr genutzt (Cleanup endgültig, veraltete Cap `barmbini_view_guide_verkaeufer` entfernt).
