...
- Interne Anleitungen `class-staff-guides.php` (Barmbini_Core_Staff_Guides): Frontend-Seiten `/anleitung-redakteur/` und `/anleitung-shop-manager/` (ausführliche Schritt-für-Schritt-Anleitungen), idempotent vom Plugin angelegt; **pro Seite** berechtigt (`barmbini_view_guide_redakteur` = Admin+Redakteur, `barmbini_view_guide_shop_manager` = Admin+Redakteur+Shop Manager) — Redakteur/Admin sehen beide, Shop Manager nur seine; Login-/Fallback-Umleitung + `noindex`; Admin-Menüpunkt „Anleitungen“ + Admin-Bar-Links als Einstieg. Seit 0.9.3 wird der alte Slug `/anleitung-verkaeufer/` nicht mehr genutzt (Cleanup endgültig, veraltete Cap `barmbini_view_guide_verkaeufer` entfernt).
