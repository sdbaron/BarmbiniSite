...
- Interne Anleitung `class-staff-guides.php` (Barmbini_Core_Staff_Guides): Frontend-Seite `/anleitung-redakteur/` (ausführliche Schritt-für-Schritt-Anleitung), idempotent vom Plugin angelegt; berechtigt mit `barmbini_view_guide_redakteur` (Admin + Redakteur); Login-/Fallback-Umleitung + `noindex`; Admin-Menüpunkt „Anleitungen“ + Admin-Bar-Link als Einstieg. Seit 0.9.1 gibt es **keine Shop-Manager-Anleitung** mehr – die frühere Seite `/anleitung-verkaeufer/` wird bei `admin_init` automatisch in den Papierkorb verschoben, die veraltete Capability `barmbini_view_guide_verkaeufer` entfernt.
