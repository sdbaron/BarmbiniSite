...
- Interne Anleitungen `class-staff-guides.php` (Barmbini_Core_Staff_Guides): Frontend-Seiten `/anleitung-redakteur/` und `/anleitung-verkaeufer/` (ausführliche Schritt-für-Schritt-Anleitungen), idempotent vom Plugin angelegt; **pro Seite** berechtigt (`barmbini_view_guide_redakteur` = Admin+Redakteur, `barmbini_view_guide_verkaeufer` = Admin+Redakteur+Shop Manager) — Redakteur sieht beide, Shop Manager nur seine; Login-/Fallback-Umleitung + `noindex`; Admin-Menüpunkt „Anleitungen“ + Admin-Bar-Links als Einstieg
