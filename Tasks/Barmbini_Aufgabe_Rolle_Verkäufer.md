...
## Nachtrag (2026-08-20): Shop-Manager-Anleitung entfernt, Redakteur-Anleitung detaillierter (0.9.1)

- Die **Shop-Manager-Anleitung** `/anleitung-verkaeufer/` wurde **komplett entfernt** – der Shop Manager ist eine WooCommerce-Standardrolle und erhält keine eigene Anleitung mehr.
- `class-staff-guides.php` (Barmbini_Core_Staff_Guides) verwaltet jetzt nur noch `/anleitung-redakteur/` (Capability `barmbini_view_guide_redakteur`, Admin + Redakteur).
- Selbstheilung: `maybe_remove_obsolete_verkaeufer_page()` verschiebt die Alt-Seite bei `admin_init` per `wp_trash_post` in den Papierkorb (reversibel); `ensure_capabilities()` entfernt `barmbini_view_guide_verkaeufer` und `barmbini_view_guides` aus allen Rollen.
