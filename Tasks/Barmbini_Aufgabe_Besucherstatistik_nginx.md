...
- **Aggregations-Korrektur (2026-08-28):** `process.php` schreibt zusätzlich die vollständigen Tages-Zähler `pages`/`referrers` (assoziativ) und normalisiert Seitenpfade **ohne Query-String**; das Plugin aggregiert daraus exakt über den Zeitraum (Fallback auf `top_pages`/`top_referrers` für ältere Tagesdateien). Die KPI „Besucher“ zeigt jetzt den **Tagesdurchschnitt** (`get_average_daily_visitors()`), da die Summe von Tages-Uniques keine eindeutige Besucherzahl über den Zeitraum ist.
- **Konfigurierbarer IP-Ausschluss (2026-08-28):** `process.php` liest `/var/lib/barmbini-stats/excluded-ips.conf` (eine IP/CIDR pro Zeile, `#`-Kommentare; Pfad per `BARMBINI_EXCLUDED_IPS_FILE` überschreibbar) und überspringt passende Client-IPs; Treffer werden als `excluded_ip_hits` gezählt. Die Liste ist auf der Admin-Seite „Statistiken“ (nur Administrator) editierbar; `install.sh` legt die Datei www-data-beschreibbar an.
- **Neu berechnen (2026-08-28):** Button „Neu berechnen“ auf der Admin-Seite (nur Administrator) setzt eine Marker-Datei `/var/lib/barmbini-stats/run/recalc.flag`; ein Root-Cron (minütlich, `recalc-queue.sh`) führt `recalc.sh` aus, das die vorhandenen Roh-Logs (live, `.1`, `.2.gz` … `.7.gz`) mit den aktuellen Filtern neu verarbeitet. `process.php` ist gzip-fähig und schreibt atomar; eine Lock (`/run/barmbini-stats.lock`) verhindert parallele Läufe. Kein direkter Root-Aufruf durch WordPress.
